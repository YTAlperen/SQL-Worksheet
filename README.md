...
SQL LEAD & LAG 1.2 Y-o-Y Growth Rate
```
with spend_cte as(
SELECT 
  ...
  lag(spend)over(
    partition by product_id order by transaction_date) as prev_year_spend
FROM user_transactions
)

select year, product_id, curr_year_spend, prev_year_spend,
round(100 * 
  (curr_year_spend - prev_year_spend) / prev_year_spend, 2) as yoy_rate
